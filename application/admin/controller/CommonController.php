	...
	public function uploadImg($fileName){
		...
		if ($file = request()->file($fileName)) {
			...
			if ($info) {
				...
				// 进行缩略图生成
				// $postData['ori_img']  20180728/hjdfgspog.png
				// $postData['thumb_img']  20180728/thumb_hjdfgspog.png
				// 1. 实例化图像类, 打开处理的原图图片的路径
				$image = \think\Image::open('./upload/'.$ori_img);
				$arr_path = explode('\\',$ori_img); // ['20180728','hjdfgspog.png']
				// 保存缩略图的路径到数据库表字段 
				$thumb_img = $arr_path[0].'/thumb_'.$arr_path[1];
				// 2.生成缩略图并进行保存起来
				$image->thumb(150,150,1)->save('./upload/'.$thumb_img);
